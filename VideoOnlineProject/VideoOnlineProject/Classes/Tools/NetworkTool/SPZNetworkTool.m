...
#import "SPZNetworkTool.h"
#import "LCNetDataParsing.h"

@implementation SPZNetworkTool

+ (SPZNetworkTool *)getInstance{
    static SPZNetworkTool *netRequest = nil;
...
+(NSString *)contectbaseHostWithPath:(NSString *)path{
    ...
    NSString *str = BaseHttpUrl;
    str = [str substringToIndex:str.length - 5];
    return [NSString stringWithFormat:@"%@%@",str,path];
}
                   ...
                   fail:(NetRequestFailedBlock)fail
{
    ...
    NSLog(@"%@\n%@",BaseUrl(urlString),dict);
    AFHTTPSessionManager *manager =  [self sharedJsonManager];
    [manager POST:BaseUrl(urlString) parameters:dict progress:^(NSProgress * _Nonnull uploadProgress) {
        
    }  success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        if (success) {
            NSError *error;
            NSData *data = [[responseObject mj_JSONString] dataUsingEncoding:NSUTF8StringEncoding];
                   ...
                   fail:(NetRequestFailedBlock)fail
{
    ...
    [manager POST:BaseUrl(urlString) parameters:[LCNetDataParsing inputParsing:dict] progress:^(NSProgress * _Nonnull uploadProgress) {
...
-(void)postDataWithUrlString:(NSString *)urlString parameters:(id)parameters success:(NetRequestSuccessBlock)success fail:(NetRequestFailedBlock)fail{
    ...
    NSURL *url = [NSURL URLWithString:BaseUrl(urlString)];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:url];
    request.HTTPMethod = @"POST";
